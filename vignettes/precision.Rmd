...
In order to address this problem, we've added the options `float_as_decimals` and `digits` to `write_dataset_json()`. When `float_as_decimals = TRUE`, the columns are written as `decimal`/`decimal` in the JSON, and `read_dataset_json()` will automatically convert them back to numeric on read per the Dataset JSON v1.1 specification.
...
There are a few reasons we've chosen to NOT make `float_as_decimals` default behavior:

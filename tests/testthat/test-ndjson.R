...
test_that("write_dataset_ndjson matches the reference ndjson files", {
  ...
  comp_meta <- yyjsonr::read_json_str(comp_lines[1])
  expected_meta <- yyjsonr::read_json_str(expected_lines[1])
...
test_that("NDJSON string output has correct structure", {
  ...
  meta <- yyjsonr::read_json_str(lines[1])
  ...
  for (i in 2:6) {
    parsed <- yyjsonr::read_json_str(lines[i])
...
test_that("float_as_decimal works with NDJSON", {
  ...
  out2 <- read_dataset_ndjson(ndjson_out2, decimals_as_float = TRUE)
